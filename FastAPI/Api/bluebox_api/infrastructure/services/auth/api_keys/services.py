...
class UserApiKeyAuthService:
    ...
    def authenticate_user(self, api_key_string: str) -> uuid.UUID:
        ...
        if response.error == api_keys_objects_pb2.AuthApiKeysError.NOT_AUTHORIZED:
            raise errors.NotAuthorizedError()
        if response.error == api_keys_objects_pb2.AuthApiKeysError.INVALID_API_KEY:
            raise errors.InvalidApiKeyError()
        if response.error == api_keys_objects_pb2.AuthApiKeysError.API_KEY_NOT_FOUND:
